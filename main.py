...
for i in range(10):
    ...
    for j in range(10):
        linha.append(0)
    main_tab.append(linha)


# tab_etapa2 = tabuleiro para os chutes das casas
tab_etapa2 = []
for i in range(10):
    linha = []
    for j in range(10):
...
tab_oculto = montar_tab_chute(tab_etapa2)
        ...
        if tab[i][j] == 0:
            ...
            for _ in range(2):
                ...
                if direcao == 1:
                    ...
                    elif (i, j-sentido) in clean_list:
                        (tab, turno_, clean_list) = ataque_mach(tab, turno, clean_list, (i,j-sentido))
                        falha = False
                        break
...
def fase1(jogador):
    ...
    tab_montado = copy.deepcopy(main_tab) # Tabuleiro em que o jogador irá montar o seu próprio em cima do tabuleiro base (main_tab)
    ...
    while True:
        ...
        if turno % 2 == 1:
            ...
            try:
                ...
                salvar_jogo(nome, turno, tab1, tab2, tab_de_jogo1, tab_de_jogo2)
                break 
...
def mach_fase1():
    ...
    while d2 + c3 + e4 + p5 != 0:
        ...
        barco = r.randint(2,5)
        direcao = r.choice([1,2]) # 1 = H e 2 = V
        ...
        match barco:
            ...
            case 5:
                ...
                if direcao == 1:
                    ...
                    if (mach_tab[i-2][j], mach_tab[i-1][j], mach_tab[i][j], mach_tab[i+1][j], mach_tab[i+2][j]) == (0,0,0,0,0):
...
def mach_fase2(nome, tab1, tab2, tab_de_jogo, turno=1, clean_list=lista_posicoes):
    ...
    while True:
        ...
        if turno % 2 == 1:
            ...
            try:
                (tab2, tab_de_jogo, turno) = ataque(1, tab2, tab_de_jogo, turno)


            except:
                salvar_jogo(nome, turno, tab1, tab2, tab_de_jogo, clean_list=clean_list)
                break 
...
def JvsJ(nome):
    tab1 = fase1(1)
    tab2 = fase1(2)
    fase2(nome, tab1, tab2, copy.deepcopy(tab_etapa2), copy.deepcopy(tab_etapa2))
    menu_login(nome)
